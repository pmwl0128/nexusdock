...
import { Activity, Archive, CheckCircle2, Layers, RefreshCw, Search, ShieldAlert, Trash2, type LucideIcon } from 'lucide-react';
...
export function TaskCenterPage({ refreshToken }: { refreshToken: number }) {
  ...
  const healthText = healthStats.blocked ? `${healthStats.blocked} 个阻塞` : healthStats.cleanable ? `${healthStats.cleanable} 个可清理` : '运行正常';
...
export function SkillsPage({ refreshToken }: { refreshToken: number }) {
  ...
  const filtered = useMemo(() => {
    ...
    return resource.data.items.filter((item) => [item.id, item.title, item.description, item.source, item.active_version, item.status].filter(Boolean).join(' ').toLowerCase().includes(needle));
  ...
  return <OpsShell title="Skill" subtitle="通过 AgentDock Runtime API 展示已安装 Skill、版本、channel、manifest 和 selection。" loading={resource.loading} error={resource.error} onReload={resource.reload}>
    ...
    <div className="ops-toolbar skill-toolbar"><label className="ops-search"><Search size={15} /><input aria-label="搜索 Skill" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索 Skill、来源、版本或状态" /></label><span className="ops-count">显示 {filtered.length} / 全部 {resource.data.count}</span></div>
    <section className="ops-master-detail skills-layout"><div className="ops-task-rail ops-skill-rail">{filtered.length === 0 ? <EmptyOps text="没有匹配的 Skill。" /> : filtered.map((skill) => <button type="button" key={`${skill.source}:${skill.id}`} className={`ops-task-line ${selected?.source === skill.source && selected?.id === skill.id ? 'is-selected' : ''}`} aria-pressed={selected?.source === skill.source && selected?.id === skill.id} onClick={() => setSelectedKey(`${skill.source}:${skill.id}`)}><span className="ops-card-icon"><Layers size={16} /></span><span><strong>{skill.title || skill.id}</strong><small>{skill.source} · {skill.active_version || 'no version'} · {skill.file_count} files · {formatTime(skill.updated_at)}</small></span><StatusBadge tone={toneForStatus(skill.status)}>{skill.status}</StatusBadge></button>)}</div><SkillDetail skill={selected} detail={detail.data.skill} loading={detail.loading} error={detail.error} /></section>
  </OpsShell>;
}
