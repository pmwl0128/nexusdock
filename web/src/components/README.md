...
当前入口位于 `web/src/App.tsx`，`MemoryWorkspace.tsx` 当前承载 RecallDock 工作区能力，文件名暂时保留以降低迁移风险。新增页面和交互应优先按领域拆分为可复用组件，避免继续扩大入口文件。
...
- Recall Explorer / Editor / Diff / Timeline 后续应从 `MemoryWorkspace.tsx` 继续拆分，但不新增独立 Inbox 或 Task 产品入口。
