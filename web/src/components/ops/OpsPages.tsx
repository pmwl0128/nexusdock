...
function SkillDetail({ skill, detail, loading, error }: { skill?: OpsSkill; detail?: OpsSkillDetail; loading: boolean; error?: string }) {
  ...
  return <article className="ops-task-detail ops-skill-detail"><header><div><span>Skill 详情</span><h3>{full.title || full.id}</h3><p>{full.description || '暂无说明。'}</p></div><StatusBadge tone={toneForStatus(full.status)}>{full.status}</StatusBadge></header>{loading && <div className="nx-alert is-info">正在读取 Skill 文档和文件清单…</div>}{error && <div className="nx-alert is-error">{error}</div>}<div className="ops-detail-grid"><Info label="ID" value={full.id} /><Info label="来源" value={full.source} /><Info label="当前版本" value={full.active_version || 'unknown'} /><Info label="文件数" value={String(full.file_count)} /><Info label="更新" value={formatTime(full.updated_at)} /><Info label="逻辑路径" value={full.path} /></div><section className="ops-detail-section"><h4>来源与版本</h4><div className="ops-key-values"><Info label="状态文件" value={full.runtime_state_path || '未读取'} /><Info label="文档目录" value={full.doc_root || detail?.root || '未读取'} /><Info label="真实目录" value={detail?.root || '未读取详情'} /><Info label="版本历史" value={(full.versions || []).slice(0, 4).join(' → ') || '暂无'} /></div><ChannelChips channels={full.channels} /></section>{detail?.skill_doc && <section className="ops-detail-section"><h4>说明文档</h4><pre className="ops-doc-preview">{detail.skill_doc}</pre></section>}<section className="ops-detail-section"><h4>文件清单</h4>{files.length === 0 ? <EmptyOps text="没有可展示文件。" /> : <div className="ops-file-list">{files.map((file) => <div key={file.path} className="ops-file-row"><span><strong>{file.path}</strong><small>{file.kind} · {formatTime(file.updated_at)}</small></span><em>{formatBytes(file.size_bytes)}</em></div>)}</div>}</section></article>;
}
