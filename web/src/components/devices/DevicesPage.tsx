...
function CommandFields({ type, fields, setFields }: { type: CommandType; fields: Record<string, string | boolean>; setFields: (value: Record<string, string | boolean>) => void }) {
  ...
  if (type === 'skill.install') return <div className="nx-form-grid"><TextField label="Skill 名称" required value={String(fields.skill ?? '')} onChange={(value) => field('skill', value)} /><TextField label="版本" value={String(fields.version ?? '')} onChange={(value) => field('version', value)} /><TextField label="发布通道" value={String(fields.channel ?? 'stable')} onChange={(value) => field('channel', value)} /></div>;
