...
export default function RecallCardsConsole({ state, actions }: Props) {
  ...
  return <section className="mem-card-console">
    <article className="mem-card-panel">
      ...
      <form className="mem-card-form" onSubmit={actions.captureCard}>
        ...
        <div className="mem-card-actions">
          ...
          {card.capture?.warnings?.length ? <label className="mem-card-check"><input type="checkbox" checked={card.allowWarnings} onChange={(event) => actions.setAllowCardWarnings(event.target.checked)} />已人工确认警告</label> : null}
        </div>
      </form>
      {card.capture && <div className="mem-card-review"><strong>{card.capture.card.path}</strong>{card.capture.warnings?.map((warning) => <p key={warning} className="warn">{warning}</p>)}{(card.capture.similar_results || []).map((item) => <button key={item.path} type="button" onClick={() => actions.openSimilarCard(item.path)}><FileText size={14} /><span>{item.title || nameOf(item.path)}</span><em>{item.path}</em></button>)}</div>}
    </article>

    <article className="mem-card-panel">
      <div className="mem-lite-panel-head">
        <div><h2>向量召回</h2><p>只搜索 cards/，用于高信噪比经验召回。</p></div>
        <span className={`mem-lite-health ${embedding.status?.reachable ? 'ok' : 'warn'}`}>{embedding.status?.reachable ? 'BGE-M3 可达' : '未就绪'}</span>
      </div>
      ...
      <form className="mem-card-search" onSubmit={actions.searchCardEmbeddings}>
        <Search size={15} />
      ...
      <div className="mem-card-results">
        {embedding.results.length === 0 ? <p className="mem-lite-empty">暂无向量搜索结果。</p> : embedding.results.map((item) => <button key={item.path} type="button" onClick={() => actions.openSimilarCard(item.path)}><strong>{item.title || nameOf(item.path)}</strong><small>{item.path}</small><em>{item.score.toFixed(4)}</em></button>)}
      </div>
    </article>
  </section>;
}
