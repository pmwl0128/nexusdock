...
type RecallEntry = { path: string; name?: string; type: EntryType; size_bytes?: number };
type Recall = { path: string; content: string };
...
type CardWriteResult = { ok: boolean; card: RecallCard; recall: Recall; warnings?: string[]; index_policy?: string };
...
export default function RecallWorkspace() {
  ...
  const [pendingAction, setPendingAction] = useState<PendingRecallAction>(null);
  ...
  useEffect(() => {
    if (!editing) return;
    const timer = window.setTimeout(() => saveRecallDraft(draftPath, draftContent), 250);
  ...
  async function openRecall(path: string) {
    ...
    setCurrent(response.recall);
    setDraftPath(response.recall.path);
    setDraftContent(response.recall.content);
    setEditing(false);
    setCreating(false);
    clearRecallDraft();
  ...
  function startNew() {
    setCurrent(null);
    setDraftPath('inbox/new-recall.md');
  ...
  function restoreDraft() {
    ...
    if (!saved) return;
    setCurrent(null);
    setDraftPath(saved.path || 'inbox/recovered-recall.md');
  ...
  async function confirmMove() {
    ...
    if (!normalized || normalized === pendingAction.path) {
      setPendingAction({ ...pendingAction, error: '请输入新的召回路径。' });
      return;
    }
  ...
  async function syncNow(action: 'pull' | 'push' | 'now' = 'now') {
    ...
    try {
      ...
      await Promise.all([loadList(), loadSyncState(), loadHistory()]);
      if (current?.path) await openRecall(current.path).catch(() => setCurrent(null));
  ...
  return (
    <main className={`mem-lite ${detailOpen ? 'is-detail-open' : ''}`}>
      ...
      {draftAvailable && !editing && <div className="mem-lite-notice"><span>检测到未提交草稿。</span><button onClick={restoreDraft}>恢复草稿</button><button onClick={() => { clearRecallDraft(); setDraftAvailable(false); }}>丢弃</button></div>}
      {pendingAction && (
        <Dialog
          title={pendingAction.kind === 'move' ? '移动召回内容' : '删除召回内容'}
          description={pendingAction.kind === 'move' ? '修改路径后会保留文件内容，并刷新当前打开的召回内容。' : '删除后会产生本地 Git 变更，需要同步后才会进入远端。'}
      ...
      <section className="mem-card-console">
        <article className="mem-card-panel">
          ...
          {cardCapture && <div className="mem-card-review"><strong>{cardCapture.card.path}</strong>{cardCapture.warnings?.map((warning) => <p key={warning} className="warn">{warning}</p>)}{(cardCapture.similar_results || []).map((item) => <button key={item.path} type="button" onClick={() => void openRecall(item.path)}><FileText size={14} /><span>{item.title || nameOf(item.path)}</span><em>{item.path}</em></button>)}</div>}
        </article>
        ...
        <article className="mem-card-panel">
          ...
          <div className="mem-card-results">
            {embeddingResults.length === 0 ? <p className="mem-lite-empty">暂无向量搜索结果。</p> : embeddingResults.map((item) => <button key={item.path} type="button" onClick={() => void openRecall(item.path)}><strong>{item.title || nameOf(item.path)}</strong><small>{item.path}</small><em>{item.score.toFixed(4)}</em></button>)}
          </div>
        </article>
      </section>
        ...
        <aside className="mem-lite-browser">
          ...
          <div className="mem-lite-files">
            {loading ? <p className="mem-lite-empty">正在读取召回内容…</p> : fileEntries.length === 0 ? <p className="mem-lite-empty">没有匹配的召回文件。</p> : fileEntries.map((entry) => (
              <button key={entry.path} className={current?.path === entry.path ? 'active' : ''} onClick={() => void openRecall(entry.path)}>
                <FileText size={16} />
        ...
        <article className="mem-lite-editor" ref={editorRef}>
          <div className="mem-lite-panel-head">
            ...
            <div className="mem-lite-editor-actions">
              ...
              {editing && <button className="primary" onClick={() => void saveRecall()} disabled={busy || !hasUnsavedChanges}><Save size={15} />保存</button>}
            </div>
          </div>
          {editing ? (
