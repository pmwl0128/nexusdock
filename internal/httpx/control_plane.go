...
import (
	...
	contracts "github.com/uvwt/nexusdock/generated/nexuscontracts"
...
func (s *Server) reportDeviceHeartbeat(w http.ResponseWriter, r *http.Request) {
	...
	if len(request.RecallSyncSummary) > 0 {
		_ = json.Unmarshal(request.RecallSyncSummary, &heartbeat.RecallSync)
	}
...
func redactCommandPayload(commandType commands.Type, payload json.RawMessage) json.RawMessage {
	if len(payload) == 0 || commandType != commands.TypeEnvManage {
