...
const defaultScheduleStatusDir = "/backup-status"
const backupScheduleID = "agentdock-nexus-backup"
...
type scheduleItem struct {
	...
	SHA256          string           `json:"sha256,omitempty"`
	RemotePath      string           `json:"remote_path,omitempty"`
	History         []scheduleStatus `json:"history"`
}

func scheduleStatusDir() string {
	if value := strings.TrimSpace(os.Getenv("NEXUS_SCHEDULE_STATUS_DIR")); value != "" {
		return filepath.Clean(value)
	}
	return defaultScheduleStatusDir
}
...
func loadScheduleItem(dir string, now time.Time) scheduleItem {
	item := scheduleItem{
		ID:           backupScheduleID,
	...
	if file, err := os.Open(historyPath); err == nil {
		...
		for scanner.Scan() {
			var entry scheduleStatus
			if json.Unmarshal(scanner.Bytes(), &entry) == nil {
				entry.State = normalizeScheduleState(entry.State)
...
func applyScheduleStatus(item *scheduleItem, status scheduleStatus) {
	item.State = normalizeScheduleState(status.State)
...
func (s *Server) listSchedules(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, http.StatusOK, map[string]any{"items": []scheduleItem{loadScheduleItem(scheduleStatusDir(), time.Now())}})
}

func (s *Server) getSchedule(w http.ResponseWriter, r *http.Request) {
	id := strings.TrimPrefix(r.URL.Path, "/v1/schedules/")
	if strings.HasPrefix(r.URL.Path, "/api/v1/schedules/") {
		id = strings.TrimPrefix(r.URL.Path, "/api/v1/schedules/")
	}
	if id != backupScheduleID {
		writeError(w, http.StatusNotFound, "SCHEDULE_NOT_FOUND", "计划任务不存在")
		return
	}
	writeJSON(w, http.StatusOK, loadScheduleItem(scheduleStatusDir(), time.Now()))
}
